...
struct func_definition_tree
{
    return_type ret_type;
    char* func_name ;
    struct stmt_list *argList ;     // UPDATE NEEDED MAY BE TO EXPR_NODE
...
// Statements
enum stmt_type {DECL_STMT = 0 , ASSIGN = 1 , READ_STMT = 2, WRITE_STMT  = 3, CONDT = 4 , RETURN_STMT = 5 , FUNC_DEF = 6 , FUNC_CALL = 7 , END_DECL = 8 , UNUSED_STMT = 9};
...
struct decl_node* createDecl_Node(decl_node_type type , char* name , int size=0 , struct declstmt_tree* args=NULL);
