...
/* FUNCTION TREE NODE */
struct funcStmt_node* createFunc_Node(arg_type type , struct node var , struct expr_node expr , struct funcStmt_node* next)
{
	...
	{
		newNode->args.var = var ;
	}
	else
	{
		printf("Error: Unsupported type of arguments for print function\n");
		exit(-1);
	}
	newNode->next = next ;
	return newNode ;
